...
def data_generate(d, gamma, alpha, Delta, sig, vlaw):
    k, n = int(gamma*d), int(alpha*d**2)
    c_k, c_d = 1/np.sqrt(k), 1/np.sqrt(d)
    if vlaw=='ones':
        v = tf.ones((k,), dtype=tf.float32)
    if vlaw=='ones_noisy':
        v = tf.ones((k,), dtype=tf.float32) + tf.random.normal((k,), dtype=tf.float32)*0.001
    if vlaw=='rad':
        v = tf.sign(tf.random.uniform((k,), -1, 1))
    if vlaw=='radd':
        l = int(k/2)
        v = tf.concat([tf.ones(l, dtype=tf.float32), -tf.ones(k-l, dtype=tf.float32)], axis=0)
    if vlaw=='unif':
        v = tf.random.uniform(shape=(k,), minval=-np.sqrt(3), maxval=np.sqrt(3), dtype=tf.float32)
    if vlaw=='unid':
        v = tf.linspace(-1,1,k)
        v = v*np.sqrt(k)/tf.norm(v)
        v = tf.cast(v, tf.float32)
    if vlaw=='gauss':
        v = tf.random.normal((k,), dtype=tf.float32)
    if vlaw=='gaussd':
